...
public class UnifiedSearchService {
  ...
  public List<VideoSearchResultDTO> performHybridSearch(SearchRequestDTO request)
    ...
    // 1. Convert text query to vector embeddings for (Semantic Search)
    List<Float> embedQueryVector = embeddingSupplier.embedQueryText(request.getQueryText());

    // 2. Build Structured Filters for Structure search
    Filter qdrantFilters = buildQdrantFilters(request.getFilters());

    // 3. Execute Hybrid Query on Qdrant
    // We ask for more results than requested because raw results are individual frames, and we will
    // group into clips later.

    int overSamplingLimit = request.getLimit() * 5;
    // Build Search request
    Points.SearchPoints searchRequest1 = Points.SearchPoints.newBuilder()
        .setCollectionName(collectionName).addAllVector(embedQueryVector).setFilter(qdrantFilters)
    ...
    SearchPoints searchRequest = SearchPoints.newBuilder().setCollectionName(collectionName)
        ...
        .addAllVector(embedQueryVector).setFilter(qdrantFilters).setLimit(overSamplingLimit)
    ...
    // Call Qdrant Client with grpc call
    ...
    // Post-Processing: Coalesce frames into video clips
   ...
   */
  private List<VideoSearchResultDTO> coalesceFramesIntoClips(List<ScoredPoint> rawFrames,
      int finalLimit) {
